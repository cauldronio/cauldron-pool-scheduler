...
from ....models.targets.git import GitRepo, GitIEnrich
...
class TestBasicCreate(TestCase):
    ...
    def test_create(self):
        ...
        enrich = GitIEnrich.objects.create(repo=repo)
        found = GitIEnrich.objects.get(repo=repo)
    ...
    def test_create_2(self):
        ...
        enrich1 = GitIEnrich.objects.create(repo=repo1)
        enrich2 = GitIEnrich.objects.create(repo=repo2)

        enriches = GitIEnrich.objects.all()
...
class TestMethods(TestCase):
    ...
    def setUp(self):
        ...
        self.enrich1 = GitIEnrich.objects.create(repo=self.repo1, user=self.user1)
        self.enrich2 = GitIEnrich.objects.create(repo=self.repo1, user=self.user2)
    ...
    def test_create_job2(self):
        """Test create GitIEnrich job with existing job"""
...
class TestSelectableIntentions(TestCase):
    def setUp(self):
        ...
        self.ienrich_U2_1 = GitIEnrich.objects.create(repo=self.repo1, user=self.user2,
                                                      status=Intention.Status.READY)
        self.user3 = User.objects.create(username='3')
        self.ienrich_U3_1 = GitIEnrich.objects.create(repo=self.repo1, user=self.user3,
                                                      status=Intention.Status.READY)
        self.ienrich_U3_2 = GitIEnrich.objects.create(repo=self.repo2, user=self.user3,
                                                      status=Intention.Status.READY)
        self.user4 = User.objects.create(username='4')
        self.ienrich_U4_1 = GitIEnrich.objects.create(repo=self.repo1, user=self.user4,
                                                      status=Intention.Status.READY)
        self.ienrich_U4_2 = GitIEnrich.objects.create(repo=self.repo2, user=self.user4,
                                                      status=Intention.Status.WORKING)

        self.user5 = User.objects.create(username='5')
        self.ienrich_U5_1 = GitIEnrich.objects.create(repo=self.repo1, user=self.user5,
                                                      status=Intention.Status.READY)
        self.job_U5 = Job.objects.create()
        self.ienrich_U5_2 = GitIEnrich.objects.create(repo=self.repo2, user=self.user5,
                                                      status=Intention.Status.READY,
                                                      job=self.job_U5)

    def test_user_1(self):
        """Test not available intentions"""
        intentions = GitIEnrich.objects.selectable_intentions(user=self.user1)
        self.assertEqual(len(intentions), 0)

    def test_user_2(self):
        """Test one intention ready"""
        intentions = GitIEnrich.objects.selectable_intentions(user=self.user2)
    ...
    def test_user_3(self):
        ...
        intentions = GitIEnrich.objects.selectable_intentions(user=self.user3, max=1)
        self.assertEqual(len(intentions), 1)
        self.assertIn(intentions[0], [self.ienrich_U3_1, self.ienrich_U3_2])

    def test_user_3b(self):
        """Test two intentions ready, get 2"""
        intentions = GitIEnrich.objects.selectable_intentions(user=self.user3, max=2)
    ...
    def test_user_4(self):
        ...
        intentions = GitIEnrich.objects.selectable_intentions(user=self.user4, max=2)
        self.assertEqual(len(intentions), 1)
        self.assertEqual(intentions[0], self.ienrich_U4_1)

    def test_user_5(self):
        """Test one ready and one ready with job"""
        intentions = GitIEnrich.objects.selectable_intentions(user=self.user5, max=2)
        self.assertEqual(len(intentions), 1)
