...
class GitHubRaw(Backend):
    ...
    def create_projects_file(self):
        ...
        logger.info("Creating projects.json for GitHubRaw repository {}".format(self.url))
        projects = {'Project': {}}
...
class GitHubEnrich(Backend):
    ...
    def create_projects_file(self):
        """Create the projects.json for Grimoirelab"""
        logger.info("Creating projects.json for GitHubEnrich repository {}".format(self.url))
        projects = {'Project': {}}
        for section in BACKEND_SECTIONS:
            projects['Project'][section] = [self.url]
        with open(PROJECTS_FILE, 'w+') as f:
